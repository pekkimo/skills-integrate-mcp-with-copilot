...
- Teacher login/logout with role-protected actions
...
| GET    | `/auth/status`                                                    | Get current teacher auth status                                     |
...
Teacher accounts are stored in `teachers.json` and loaded by the backend at startup.
